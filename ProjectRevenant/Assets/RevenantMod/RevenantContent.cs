...
namespace RevenantMod
{
    ...
    public class RevenantContent : IContentPackProvider
    {
        ...
        IEnumerator IContentPackProvider.LoadStaticContentAsync(LoadStaticContentAsyncArgs args)
        {
            ...
            _initialized = true;
            var enumerator = RevenantAssets.Initialize(); //We initialize our assetbundles and await them.
        ...
        {
            RevenantMain main = RevenantMain.instance; //get a direct reference to our plugin for ease of access
            _loadDispatchers = new Func<IEnumerator>[] //Create our array that will initialize our mod.
            {
                () => 
                {
                    ...
                    ItemModule.AddProvider(main, ContentUtil.CreateGenericContentPieceProvider<ItemDef>(main, revenantContentPack));
            ...
            {
                () => ContentUtil.PopulateTypeFields(typeof(Items), revenantContentPack.itemDefs),
            };
        }
